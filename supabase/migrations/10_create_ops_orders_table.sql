-- Create ops_orders table for storing operational orders (delivery, installation, fabrication)
...
CREATE INDEX idx_ops_orders_sale_id ON ops_orders(sale_id);
CREATE INDEX idx_ops_orders_status ON ops_orders(status);
CREATE INDEX idx_ops_orders_type ON ops_orders(order_type);
CREATE INDEX idx_ops_orders_scheduled_date ON ops_orders(scheduled_date);
